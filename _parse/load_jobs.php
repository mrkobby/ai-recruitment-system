...
while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
	...
	while ($row = mysqli_fetch_array($_query, MYSQLI_ASSOC)) {
		...
		while ($row = mysqli_fetch_array($query1, MYSQLI_ASSOC)) {
			...
			if($is_active == 0){
				$rjoblist .= '<div id="jb_'.$job_id.'" class="job"><div class="job-user-image hand" onclick="recruiter(\''.$comp_id.'\')" style="vertical-align: inherit;">'.$company_pic.'</div>';
				$rjoblist .= '<div class="job-details"><a href="javascript:void(0)" onclick="recruiter(\''.$comp_id.'\')"><h3>'.$company_name.'</h3></a>';
				...
				$rjoblist .= '<div id="jb_'.$job_id.'" class="job" style="background-color: #fee3e3;"><div class="job-user-image hand" onclick="recruiter(\''.$comp_id.'\')" style="vertical-align: inherit;">'.$company_pic.'</div>';
				$rjoblist .= '<div class="job-details"><a href="javascript:void(0)" onclick="recruiter(\''.$comp_id.'\')"><h3>'.$company_name.'</h3></a>';
				$rjoblist .= '<h4 style="margin-top:5px;"><span class="grey-out">Job title:</span> '.$job_title.'</h4> ';
				$rjoblist .= '<h4 style="margin-top:5px;"><span class="grey-out">Job type:</span> '.$job_type.'</h4> ';
				$rjoblist .= '<h4 style="margin-top:5px;"><span class="grey-out">Posted on:</span> '.$created_date.'</h4> ';
